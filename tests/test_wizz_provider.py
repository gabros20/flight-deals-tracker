...
# --------------------------------------------------------------------------- #
@responses.activate
def test_timetable_happy_both_directions_and_fx():
    ...
    f = result.outbound[0]
    assert f.carrier == "wizzair"
    assert f.price_confidence == "approximate"
    assert f.currency_original == "HUF"
    assert f.source_endpoint == "wizz/timetable"
    # 26090 HUF at seed rate 395 -> EUR
    assert f.price_eur == round(26090.0 / 395.0, 2) == 66.05
    # Everything normalized to EUR (nothing non-EUR leaks past the boundary).
    assert all(f.price_eur > 0 for f in result.outbound + result.inbound)
